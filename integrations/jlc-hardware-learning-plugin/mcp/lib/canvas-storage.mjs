...
const CANVAS_FILE_NAME = "hardware-learning-canvas.json";
const LEGACY_CANVAS_FILE_NAME = "cowart-canvas.json";
const SELECTION_FILE_NAME = "hardware-learning-selection.json";
...
export function resolveHardwareLearningPaths(args = {}) {
  ...
  const envProjectDir = nonEmptyString(
    process.env.JLC_HARDWARE_LEARNING_PROJECT_DIR || process.env.COWART_PROJECT_DIR,
  );
  const envCanvasDir = nonEmptyString(
    process.env.JLC_HARDWARE_LEARNING_CANVAS_DIR || process.env.COWART_CANVAS_DIR,
  );
...
export function resolveCanvasDir(args = {}) {
  return resolveHardwareLearningPaths(args).canvasDir;
}
...
function migrateLegacyHardwareLearningSnapshot(snapshot) {
  if (!snapshot?.store) return snapshot;
  const migrated = cloneJson(snapshot);
  for (const record of Object.values(migrated.store)) {
    ...
    for (const [legacyPrefix, canonicalPrefix] of LEGACY_METADATA_PREFIXES) {
      for (const key of Object.keys(record.meta)) {
        ...
        if (!Object.hasOwn(record.meta, canonicalKey)) record.meta[canonicalKey] = record.meta[key];
...
async function readJsonFileWithLegacy(primaryPath, legacyPath) {
  try {
    return { value: await readJsonFile(primaryPath), path: primaryPath, legacy: false };
  } catch (error) {
    if (error?.code !== "ENOENT") throw error;
  }
  return { value: await readJsonFile(legacyPath), path: legacyPath, legacy: true };
}
...
async function readPageSnapshots(args = {}) {
  ...
  if (manifest) {
    ...
    for (const page of manifest.pages) {
      const loaded = await readJsonFileWithLegacy(
        pageFilePath(args, page.id),
        legacyPageFilePath(args, page.id),
      );
      const { value: snapshot, path: filePath } = loaded;
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (isCanvasSnapshot(loaded.value)) {
        snapshots.push({ filePath: loaded.path, snapshot: loaded.value });
      }
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
    }
...
async function loadStoredCanvasSnapshot(args = {}) {
  ...
  if (pageSnapshots.length > 0) {
    ...
    return {
      snapshot: migrateLegacyHardwareLearningSnapshot(mergedSnapshot),
  ...
  try {
    const loaded = await readJsonFileWithLegacy(canvasFile(args), legacyCanvasFile(args));
    return {
      snapshot: migrateLegacyHardwareLearningSnapshot(loaded.value),
      path: loaded.path,
      storage: loaded.legacy ? "legacy-single-file" : "single-file",
    };
  } catch (error) {
...
async function saveStoredCanvasSnapshot(args, snapshot) {
  ...
  if (pages.length === 0) {
    ...
    return { storage: "single-file", paths: [canvasFile(args)] };
  }
...
export async function writeHardwareLearningPageAsset(args = {}, options = {}) {
  const pageId = nonEmptyString(options.pageId);
  if (!pageId) throw new Error("pageId is required to save a JLC Hardware Learning page asset.");
  ...
  if (!mimeType.startsWith("image/")) {
    throw new Error(`JLC Hardware Learning page assets only accept image payloads. Received ${mimeType}.`);
  }

  const canvasDir = resolveCanvasDir(args);
  const destinationDir = pageAssetsDir(args, pageId);
  if (!isSafeChildPath(canvasDir, destinationDir)) {
    throw new Error(`Unsafe JLC Hardware Learning page assets directory: ${destinationDir}`);
  }
...
export async function readHardwareLearningPageAsset(args = {}, options = {}) {
  const assetUrl = nonEmptyString(options.assetUrl);
  if (!assetUrl) throw new Error("assetUrl is required to read a JLC Hardware Learning page asset.");
  if (!assetUrl.startsWith(PAGE_ASSETS_ROUTE) && !assetUrl.startsWith(GLOBAL_ASSETS_ROUTE)) {
    throw new Error(`Unsupported JLC Hardware Learning asset URL: ${assetUrl}`);
  }

  const filePath = localAssetFilePathFromUrl(assetUrl, args);
  if (!filePath) throw new Error(`Unsafe JLC Hardware Learning asset URL: ${assetUrl}`);

  const fileStat = await stat(filePath);
  if (!fileStat.isFile()) throw new Error(`JLC Hardware Learning asset is not a file: ${assetUrl}`);

  const mimeType = mimeTypes.get(extname(filePath).toLowerCase()) || "application/octet-stream";
  if (!mimeType.startsWith("image/")) {
    throw new Error(`JLC Hardware Learning page assets only expose image payloads. Received ${mimeType}.`);
  }
...
export async function readHardwareLearningCanvasState(args = {}, { hydrateAssets = false } = {}) {
  const { projectDir, canvasDir } = resolveHardwareLearningPaths(args);
  ...
  const { viewState, viewStateFile } = await readHardwareLearningViewState(args);
...
export async function ensureHardwareLearningCanvasState(args = {}) {
  const loaded = await loadStoredCanvasSnapshot(args);
...
export async function saveHardwareLearningCanvasSnapshot(args = {}, snapshot) {
  const { sanitizeCanvasSnapshotForTldraw } = await import("../../src/canvasSnapshot.js");
  const sanitized = sanitizeCanvasSnapshotForTldraw(
    migrateLegacyHardwareLearningSnapshot(snapshot),
  );
  ...
  if (imageLosses.length > 0) {
    return {
      ...
      message: `JLC Hardware Learning refused to save because ${imageLosses.length} existing image shape(s) disappeared without a user delete confirmation.`,
    };
  }
...
export async function readHardwareLearningSelectionState(args = {}) {
  const selectionFile = resolveSelectionFile(args);
  try {
    const loaded = await readJsonFileWithLegacy(selectionFile, resolveLegacySelectionFile(args));
    const selection = loaded.value;
    if (!isSelectionState(selection)) {
      throw new Error(`Invalid selection state in ${loaded.path}`);
    }
    return { selection, selectionFile: loaded.path, canonicalSelectionFile: selectionFile };
  } catch (error) {
    if (error?.code === "ENOENT") {
      return {
...
export async function writeHardwareLearningSelectionState(args = {}, selection) {
  if (!isSelectionState(selection)) {
    throw new Error("Expected a JLC Hardware Learning selection state.");
  }
  const selectionFile = resolveSelectionFile(args);
  let previousSelection = null;
  try {
    previousSelection = (
      await readJsonFileWithLegacy(selectionFile, resolveLegacySelectionFile(args))
    ).value;
  } catch (error) {
    if (error?.code !== "ENOENT") throw error;
  }
...
export async function readHardwareLearningViewState(args = {}) {
  const viewStateFile = resolveViewStateFile(args);
  try {
    const loaded = await readJsonFileWithLegacy(viewStateFile, resolveLegacyViewStateFile(args));
    const viewState = loaded.value;
    return {
      viewState: isViewState(viewState) ? viewState : defaultViewState(),
      viewStateFile: loaded.path,
      canonicalViewStateFile: viewStateFile,
    };
  } catch (error) {
    if (error?.code === "ENOENT") {
...
export async function writeHardwareLearningViewState(args = {}, viewState) {
  if (!isViewState(viewState)) {
    throw new Error("Expected a JLC Hardware Learning view state.");
  }
  const viewStateFile = resolveViewStateFile(args);
  const payload = {
    ...viewState,
