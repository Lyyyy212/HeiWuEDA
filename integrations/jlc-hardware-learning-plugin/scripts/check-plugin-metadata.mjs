...
assert.equal(marketplace.name, "jlc-hardware-learning-local");
const marketplacePlugin = marketplace.plugins?.find(({ name }) => name === pluginManifest.name);
assert.ok(marketplacePlugin, "marketplace must include the JLC Hardware Learning plugin");
assert.equal(marketplacePlugin.source?.source, "local");
assert.ok(
  marketplacePlugin.source?.path === "." || marketplacePlugin.source?.path === "./",
  "JLC Hardware Learning marketplace source must point at the repository root",
);
...
console.log(`JLC Hardware Learning plugin metadata OK (${pluginManifest.version})`);
