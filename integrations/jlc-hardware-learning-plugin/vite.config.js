...
const projectDir = resolve(
  process.env.JLC_HARDWARE_LEARNING_PROJECT_DIR ??
  process.env.COWART_PROJECT_DIR ??
  process.cwd()
)
...
const canvasDir = resolve(
  process.env.JLC_HARDWARE_LEARNING_CANVAS_DIR ??
  process.env.COWART_CANVAS_DIR ??
  join(projectDir, 'canvas')
)
const canvasFile = join(canvasDir, 'hardware-learning-canvas.json')
const legacyCanvasFile = join(canvasDir, 'cowart-canvas.json')
const selectionFile = join(canvasDir, 'hardware-learning-selection.json')
const legacySelectionFile = join(canvasDir, 'cowart-selection.json')
const viewStateFile = join(canvasDir, 'hardware-learning-view-state.json')
...
const canvasFileName = 'hardware-learning-canvas.json'
...
function isHtmlDraftShapeRecord(record) {
  return record?.typeName === 'shape' && record.type === 'embed' && record.meta?.hardwareLearningHtmlDraft === true
}
...
async function updateHtmlDraftSnapshot(snapshot, { draftShapeId, htmlContent }) {
  ...
  const existingAssetUrl = shape.meta?.hardwareLearningHtmlDraftAssetUrl
  ...
  const sharedAsset =
    ...
    Object.values(snapshot.store).some(
      (record) =>
        ...
        record.meta?.hardwareLearningHtmlDraftAssetUrl === existingAssetUrl
    )
...
async function persistHtmlDraftAsset(record, pageId) {
  if (
    ...
    record.type !== 'embed' ||
    record.meta?.hardwareLearningHtmlDraft !== true ||
    typeof record.props?.url !== 'string'
  ) {
    return record
  }

  const assetUrl = record.meta?.hardwareLearningHtmlDraftAssetUrl
  const expectedPrefix = `${pageAssetsRoute}${pageDirName(pageId)}/`
...
async function loadCanvasSnapshot() {
  ...
  try {
    const loaded = await readJsonFileWithLegacy(canvasFile, legacyCanvasFile)
    return {
...
function canvasStoragePlugin() {
  return {
    name: 'jlc-hardware-learning-canvas-storage',
    configureServer(server) {
      ...
      server.middlewares.use('/api/html-draft', async (req, res) => {
        try {
          ...
          if (!loaded.snapshot) {
            sendJson(res, 404, { error: 'No JLC Hardware Learning canvas snapshot exists.' })
            return
          }
      ...
      server.middlewares.use('/api/selection', async (req, res) => {
        try {
          if (req.method === 'GET') {
            try {
              sendJson(res, 200, {
                selection: (await readJsonFileWithLegacy(selectionFile, legacySelectionFile)).value,
                path: selectionFile
              })
            } catch (error) {
              if (error.code === 'ENOENT') {
          ...
          if (req.method === 'PUT') {
            ...
            if (!isSelectionState(selection)) {
              sendJson(res, 400, { error: 'Expected a JLC Hardware Learning selection state.' })
              return
            }
      ...
      server.middlewares.use('/api/view-state', async (req, res) => {
        try {
          if (req.method === 'GET') {
            try {
              sendJson(res, 200, {
                viewState: (await readJsonFileWithLegacy(viewStateFile, legacyViewStateFile)).value,
                path: viewStateFile
              })
            } catch (error) {
              if (error.code === 'ENOENT') {
          ...
          if (req.method === 'PUT') {
            ...
            if (!isViewState(viewState)) {
              sendJson(res, 400, { error: 'Expected a JLC Hardware Learning view state.' })
              return
            }
...
export default defineConfig({
  ...
  define: {
    __JLC_HARDWARE_LEARNING_APP_VERSION__: JSON.stringify(appVersion),
